...
## Detecting weather Offset is needed
For some cheap displays, the controller resolution may be configured to 132x162 pixels, that exceeds the panel's of 128x160 pixels. This can be detected automatically using the function `lcd_st7735_check_offset`.
...
void main(void){
    ...
    size_t x = 0, y = 0;
    Result res = lcd_st7735_check_offset(&lcd, &x, &y);
    if(res.code != 0){
        LOG("Warning: Unable to determine LCD offset. Try slower SPI clock.");
    }else if(x != 0){
        LOG("Offset needed");
        lcd_st7735_set_offset(&lcd, x,y); 
    }else{
