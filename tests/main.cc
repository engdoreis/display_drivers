...
#define GET_GOLDEN_FILE()                                                                                              \
  std::format("./tests/golden_files/test_{}.bmp", testing::UnitTest::GetInstance()->current_test_info()->name())
  ...
  void compare_bmp(const std::string &result_bmp, const std::string &expected_bmp) {
    int w1, h1, c1, w2, h2, c2;

    unsigned char *result   = stbi_load(result_bmp.c_str(), &w1, &h1, &c1, 0);
    unsigned char *expected = stbi_load(expected_bmp.c_str(), &w2, &h2, &c2, 0);

    ASSERT_NE(expected, nullptr) << std::format("File {} not found, to compare with {}", expected_bmp, result_bmp);
    ASSERT_NE(result, nullptr) << std::format("File {} not found, to compare with {}", result_bmp, expected_bmp);
    ASSERT_TRUE(w1 == w2 && h1 == h2 && c1 == c2);

    size_t size = w1 * h1 * c1;
    ASSERT_TRUE(std::memcmp(result, expected, size) == 0)
        << std::format("Mismatch {} != {}\n", result_bmp, expected_bmp)
        << std::format("Run: compare {} {} /tmp/diff.png", result_bmp, expected_bmp);
...
TEST_F(st7735SimTest, draw_rectangles) {
  ...
  mock_.simulator.bmp(filename);
  compare_bmp(filename, GET_GOLDEN_FILE());
}
